...
const image_1 = __importDefault(require("../../src/export/image"));
jest.mock("puppeteer", () => {
...
describe("launchExporter", () => {
    it("forwards browser console messages", async () => {
        expect.assertions(2);
        const { page } = await (0, src_1.launchExporter)();
...
describe("render", () => {
    it("calls render and retrieves result information from the DOM", async () => {
        expect.assertions(2);
        const args = ["", 0, src_1.Format.PNG];
        const { page } = await (0, src_1.launchExporter)({ debug: true });
        await (0, browser_1.render)(page, true, ...args);
...
describe("exportImage", () => {
    it("scales the viewport according to the bounds", async () => {
        expect.assertions(2);
        const exporter = await (0, src_1.launchExporter)({ debug: true });
        await (0, image_1.default)(exporter, "", 0, src_1.Format.PNG);
    ...
    it("closes the browser without invoking the timeout callback", async () => {
        ...
        expect.assertions(3);
        const callback = jest.fn();
        const exporter = await (0, src_1.launchExporter)({
            timeout: 500,
            callback,
            debug: true
        });
...
//# sourceMappingURL=diagrams.test.js.map